    ...
    <Card className="group overflow-hidden transition-all hover-elevate" data-testid={`card-product-${id}`}>
      ...
      <div className="space-y-2 sm:space-y-3 p-3 sm:p-4">
        <div>
          ...
          <h3 className="mt-1 font-display text-base sm:text-lg font-semibold leading-tight" data-testid={`text-product-name-${id}`}>
            {name}
          </h3>
        </div>
        <div className="flex items-center justify-between gap-2">
          <span className="font-display text-xl sm:text-2xl font-bold text-primary" data-testid={`text-price-${id}`}>
